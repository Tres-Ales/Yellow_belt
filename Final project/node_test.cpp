...
void TestDateComparisonNode() {
  const Date november_1{2019, 10, 1};
  const Date november_2{2019, 10, 2};
  const Date november_3{2019, 10, 3};
...
void TestLogicalOperationNode() {
  ...
  }
  {
    LogicalOperationNode root(
      ...
      make_shared<DateComparisonNode>(Comparison::Equal, Date{2019, 10, 18}),
      make_shared<EventComparisonNode>(Comparison::Equal, "Saturday")
    );

    Assert(root.Evaluate({2019, 10, 18}, "Saturday"), "LogicalOperationNode propagates arguments 1");
    Assert(!root.Evaluate({2019, 10, 18}, "Friday"), "LogicalOperationNode propagates arguments 2");
    Assert(!root.Evaluate({2019, 10, 1}, "Saturday"), "LogicalOperationNode propagates arguments 1");
  }
}
